...
- [x] 15 — SPA layout (Sakai-style shell, router, role menu, error pages)
  ...
  - **(2026-06-02 follow-up) Stub sections now built.** Every ComingSoon menu route was replaced with a real UI. A config-driven generic CRUD engine (`resources/spa/views/crud/`: CrudListView + CrudFormView + `configs.js` + `helpers.js`) powers 12 sections (insurance-vehicles, documents, gateways, admin services/service-fields/service-groups/sub-plans/sub-requests/user-subscriptions/plan-fees/users/notifications-manage); bespoke views cover Subscription, admin Settings, SAFER Web (tabbed inspections/crashes), Orders (list+detail+pay), To-Do (entity-switching filters), Request Manage (edit+status+delete). **Clearing House stays a placeholder** — no `/api/v1` backend exists (step 11 not done); its ComingSoon now shows an accurate message. Route + menu role gating aligned to the API (Orders list & notifications-manage tightened to match `hasRole`). Built via 3 workflows (contract-map → build → adversarial review); the review found 5 real issues, all fixed: admin Users role edit object↔slug (`valuePath` config hook), 2 dead status filters (added `where()` to `SubManagerActions`/`SubRequestActions::index`), and identity fields silently dropped on edit (`createOnly` config flag). `npm run build` green; full suite **273 passed**.
